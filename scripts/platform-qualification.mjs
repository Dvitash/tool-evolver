...
export const REQUIRED_QUALIFICATION_LANES = Object.freeze([
  "linux-x64",
  "linux-arm64",
  "darwin-x64",
  "darwin-arm64",
...
function extractRelease(archivePath, targetDir) {
  ...
  const installedRoot = path.join(targetDir, "tool-evolver");
  if (!fs.existsSync(installedRoot)) {
    throw new Error("Release artifact did not contain tool-evolver root directory");
  }
...
function validatePlatformMetadata(installedRoot, lane, manifest) {
  ...
  const expectedWsl = lane === "wsl";
  const expectedPlatform = lane.startsWith("darwin") ? "darwin" : "linux";
  const expectedArch = lane.endsWith("arm64") ? "arm64" : "x64";
  if (
...
function qualifyCli(installedRoot, sandboxDir, manifest) {
  ...
  if (version.status !== 0 || !version.stdout.includes("tool-evolver v")) {
  ...
  if (help.status !== 0 || !help.stdout.includes("Tool Evolver CLI")) {
...
async function qualifyDaemon(installedRoot, sandboxDir) {
  const daemonBin = path.join(installedRoot, "bin", "tool-evolver-daemon");
  ...
  const env = {
    ...process.env,
    NODE_ENV: "production",
    TOOL_EVOLVER_LOG_LEVEL: "silent",
    TOOL_EVOLVER_CLOUD_SYNC_ENABLED: "false",
    TOOL_EVOLVER_TELEMETRY_ENABLED: "false",
  };
  delete env.NODE_PATH;
...
async function qualifyMcp(installedRoot, sandboxDir) {
  const mcpBin = path.join(installedRoot, "bin", "tool-evolver-mcp");
  ...
  try {
    ...
    if (!toolNames.includes("echo")) {
      throw new Error(`MCP catalog did not include echo: ${JSON.stringify(toolNames)}`);
    }
    const called = await rpc.request(3, "tools/call", {
      name: "echo",
      arguments: { message: "platform-qualification" },
    });
    if (called.error) throw new Error(`MCP tools/call failed: ${JSON.stringify(called)}`);
    const rendered = JSON.stringify(called.result ?? {});
    if (!rendered.includes("platform-qualification")) {
      throw new Error(`MCP echo invocation returned unexpected result: ${rendered}`);
    }
    return {
      initialized: true,
      catalogRefresh: true,
...
async function probeHarnesses(installedRoot) {
  const probes = [
    {
      id: "claude-code",
      module: path.join(installedRoot, "adapters", "claude-code", "dist", "index.js"),
      fn: "probeClaudeInstallation",
    },
    {
      id: "codex-cli",
      module: path.join(installedRoot, "adapters", "codex-cli", "dist", "index.js"),
      fn: "probeCodexInstallation",
    },
    {
      id: "omp",
      module: path.join(installedRoot, "adapters", "omp", "dist", "index.js"),
      fn: "probeOmpInstallation",
    },
  ];
  const results = [];
...
export async function qualifyPlatformLane(lane, options = {}) {
  ...
  const sandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), `tool-evolver-${lane}-qualification-`));
  const baseEvidence = {
    schemaVersion: "2.0.0",
    lane,
    host,
  ...
  try {
    ...
    const harnesses = await probeHarnesses(installedRoot);
