 ...
 *
 * Measures controlled multi-turn baseline execution against autonomously
 * evolved single-tool execution across 3 repeated trials, comparing:
 *  1. Total Model Tokens (Prompt + Completion)
...
console.log("================================================================================");
console.log("       TOOL EVOLVER V1.0.0 — AUTONOMOUS EVOLUTION BENCHMARK (3 TRIALS)         ");
console.log("================================================================================");
...
async function runBenchmark() {
  ...
  const trials = 3;
  const baselineSessions = 3;
  const stepsPerSession = 3;
  const totalBaselineTurns = baselineSessions * stepsPerSession; // 9 turns total

  // Baseline multi-turn token calculations per turn:
  const baselinePromptTokens = totalBaselineTurns * probePromptTokens;
  const baselineCompletionTokens = totalBaselineTurns * probeCompletionTokens;
  ...
  // Standard market pricing equivalent ($0.003/1k input, $0.015/1k output)
  const baselineCostUsd = (baselinePromptTokens / 1000) * 0.003 + (baselineCompletionTokens / 1000) * 0.015;

  console.log(`▶ STEP 1: Baseline Multi-Turn Workflow Reference`);
  console.log(`  • Un-evolved Task  : 3 sessions × 3 conversational turns each (${totalBaselineTurns} turns total)`);
  console.log(`  • Baseline Tokens  : ${totalBaselineTokens.toLocaleString()} tokens`);
  console.log(`  • Baseline Latency : ${(totalBaselineLatencyMs / 1000).toFixed(2)}s cumulative model wait time`);
  console.log(`  • Baseline Cost    : $${baselineCostUsd.toFixed(4)} (market model equivalent)\n`);

  console.log(`▶ STEP 2: Running Autonomous Evolution & Tool Invocation (${trials} Iterations)...`);
  ...
  for (let i = 1; i <= trials; i++) {
    ...
    console.log(`  ✓ Trial ${i}/${trials}: Pipeline completed in ${(evoDuration / 1000).toFixed(2)}s, Tool invoked in ${invLatency.toFixed(2)}ms (Success: ${invRes.success})`);
    await env.shutdown();
  }
  ...
  // Single-turn tool invocation footprint:
  ...
  const tokenSavingsPct = ((tokenSavings / totalBaselineTokens) * 100).toFixed(1);

  const latencySavingsMs = totalBaselineLatencyMs - avgInvocationMs;
  const latencySavingsPct = ((latencySavingsMs / totalBaselineLatencyMs) * 100).toFixed(1);

  const costSavingsUsd = baselineCostUsd - evolvedCostUsd;
  const costSavingsPct = ((costSavingsUsd / baselineCostUsd) * 100).toFixed(1);
  ...
  console.log("|------------------------|----------------------------|-------------------------|-------------------|");
  console.log(`| Conversational Turns   | ${totalBaselineTurns} turns                   | 1 turn                  | -${turnSavings} turns (${turnSavingsPct}%) |`);
  ...
  console.log("================================================================================");
  console.log(`\n• Autonomous Synthesis Time : ${(avgEvolutionMs / 1000).toFixed(2)}s average across ${trials} runs`);
  console.log(`• Evolved Tool Local Latency : ${avgInvocationMs.toFixed(2)}ms average (in-memory sandboxed execution)`);
