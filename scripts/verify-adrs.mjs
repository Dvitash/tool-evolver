 ...
 */
export function validateSequentialNumbering(adrs) {
  /** @type {AdrViolation[]} */
  const violations = [];
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    // Check strictly sequential
    if (adr.number !== expectedNum) {
      violations.push({
        rule: "SEQUENCE",
        file: adr.file,
        message: `ADR sequence gap: expected 000${expectedNum}, but found ${adr.numberStr}.`,
      });
    }
...
export function verifyAdrs(rootDir = process.cwd()) {
  ...
  violations.push(...validateSequentialNumbering(parsedAdrs));
