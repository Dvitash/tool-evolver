#!/usr/bin/env bash
# ==============================================================================
# Tool Evolver - Branch Protection & Release Gate Configuration Script
#
# Configures GitHub branch protection rules on 'main' to enforce PR-only release gates:
# 1. Require Pull Request reviews with code owner signoff.
# 2. Dismiss stale approvals on new commits.
# 3. Require branches to be up to date before merging.
# 4. Require all 10 parallel CI status checks + rollup 'CI Gate Rollup' to pass.
...
Configures strict branch protection rules for Tool Evolver repositories.
...
PROTECTION_PAYLOAD=$(cat <<EOF
{
  "required_status_checks": {
    ...
    "contexts": [
      ...
      "Unit & Contract Tests",
      "End-to-End Tests",
      "Package Import Boundary Enforcement",
      "ADR Validation",
      "Release Packaging & Integrity Verification",
      "Binary Smoke Tests",
      "Secret Scanning",
      "CI Gate Rollup"
    ]
