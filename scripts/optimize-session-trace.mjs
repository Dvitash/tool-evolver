...
for (let i = 0; i < turns.length; i++) {
  const t = turns[i];
  const isMechanical =
    t.toolNames.some((n) => ["glob", "grep", "read", "edit", "write"].includes(n)) ||
    t.role === "toolResult";
  ...
  if (isMechanical) {
    ...
    if (currentSeq.length >= 6) {
      // Block of 3+ tool call/result pairs
...
console.log(
  `  ✓ Identified ${mechanicalSequences.length} repetitive sub-workflow clusters spanning ${totalMechanicalTurns} turns.`,
);
...
console.log("================================================================================");
console.log(
  "| Metric                 | Original Session Trace     | With Evolved Sub-Tools  | Realistic Net Savings |",
);
console.log(
  "|------------------------|----------------------------|-------------------------|-----------------------|",
);
console.log(
  `| Total Session Turns    | ${turns.length.toString().padEnd(26)} | ${optimizedTurnsCount.toString().padEnd(23)} | -${turnSavings} turns (${turnSavingsPct}%)   |`,
);
console.log(
  `| Total Cumulative Tokens| ${totalBaselineTokens.toLocaleString().padEnd(26)} | ${totalOptimizedTokens.toLocaleString().padEnd(23)} | -${tokenSavings.toLocaleString()} (${tokenSavingsPct}%) |`,
);
console.log(
  `| Estimated Model Cost   | $${baselineCostUsd.toFixed(4).padEnd(25)} | $${optCostUsd.toFixed(4).padEnd(22)} | -$${costSavingsUsd.toFixed(4)} (${costSavingsPct}%)  |`,
);
console.log("================================================================================");
console.log(
...
console.log(
  `• Mechanical Loops Replaced          : ${mechanicalSequences.length} sub-workflows (${totalMechanicalTurns} turns consolidated into ${mechanicalSequences.length * 2} tool turns)`,
);
console.log(
  `• Data Provenance                    : Calculated directly from transcript character counts and real context window growth.\n`,
);
