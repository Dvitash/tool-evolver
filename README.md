...
3. `pnpm run lint` — Biome formatting and linter validation
4. `pnpm run typecheck` — TypeScript strict type validation
5. `pnpm run build` — Topological build across all workspace packages
6. `pnpm run test` — Unit test suite execution
7. `pnpm run test:e2e` — End-to-end test execution
8. `pnpm run check:smoke` — Binary smoke execution for CLI, Daemon, Gateway MCP, and Conformance Runner
9. `pnpm run release:verify` — Release tarballs, SHA-256 digests, Ed25519 signatures, SBOM, and docs cross-links
