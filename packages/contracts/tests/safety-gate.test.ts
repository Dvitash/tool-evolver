...
describe("Safety Gate Contracts & Schemas", () => {
  it("exports expected version constants and error codes", () => {
    ...
    expect(UNSAFE_DEV_OVERRIDE_ENV_VAR).toBe("TOOL_EVOLVER_UNSAFE_ALLOW_AUTONOMOUS");
    expect(UNSAFE_ENV_PREFIX).toBe("TOOL_EVOLVER_UNSAFE_");
      ...
      unmetRequirements: [
        {
          ...
          remediation: "Run 'tool-evolver doctor --repair' to generate a valid attestation.",
        },
      ],
  ...
  it("validates SafetyGateRefusalSchema", () => {
    const refusal = {
      ...
      remediation: "Run 'tool-evolver doctor --repair' or install a valid safety attestation.",
