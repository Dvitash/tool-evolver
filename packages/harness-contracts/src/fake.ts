...
import type { HarnessAdapter, ProbeInstallationOptions } from "./adapter.js";
import {
  type ConfigFsBridge,
...
import { TIER1_HIGH_FIDELITY } from "./fidelity.js";
import { type RefreshOutcome, type RefreshResult, createRefreshResult } from "./refresh.js";
import type { RecordListener, SessionEventSource } from "./source.js";
import type {
  AdapterCapabilities,
...
export class FakeHarnessAdapter implements HarnessAdapter {
  ...
  async planMcpConfig(
    ...
    return planConfigMutation({
      ...
      currentContent,
      plannedContent,
      description: `Add Tool Evolver Gateway MCP server at ${gatewayUrl}`,
    });
  }
  ...
  async verifyMcpConfig(workspace: HarnessWorkspace): Promise<boolean> {
    ...
    try {
      ...
      return Boolean(parsed.mcpServers?.["tool-evolver"]);
    } catch {
      return false;
    }
  }
