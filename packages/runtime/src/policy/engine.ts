...
export class CapabilityPolicyEngine {
  ...
  evaluateInvocation(
    ...
    // 4. Extract CapabilityManifest
    let requestedCapabilities: CapabilityManifest;
    let rawCapabilityObject: unknown = manifestOrCapabilities;

    if (
      manifestOrCapabilities &&
      typeof manifestOrCapabilities === "object" &&
      "capabilities" in (manifestOrCapabilities as Record<string, unknown>)
    ) {
      const toolManifest = manifestOrCapabilities as ToolManifest;
      requestedCapabilities = toolManifest.capabilities;
      rawCapabilityObject = toolManifest.capabilities;
    } else {
