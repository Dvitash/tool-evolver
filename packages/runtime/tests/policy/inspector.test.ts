...
describe("Policy Inspector", () => {
  ...
  describe("Remediation CLI Commands", () => {
    it("generates actionable CLI remediation commands for violations", () => {
      ...
      expect(commands).toContain("tool-evolver envelope expand --add-domain api.service.com");
      expect(commands).toContain('tool-evolver envelope expand --add-write-path "dist/build/**"');
      expect(commands).toContain("tool-evolver envelope expand --add-command cargo");
      expect(commands).toContain("tool-evolver envelope expand --add-secret STRIPE_API_KEY");
    });
  });
