...
export class FutureMigrationError extends Error {
  constructor(dbVersion: number, codeVersion: number) {
    super(
      `Database schema version (${dbVersion}) is newer than codebase maximum supported version (${codeVersion}). Downgrades are not supported; update tool-evolver software.`,
    );
