...
export class EncryptedVaultSecretStore implements SecretStore {
  ...
  constructor(options: EncryptedVaultOptions = {}) {
    ...
    const rawPassphrase =
      options.passphrase ??
      process.env.TOOL_EVOLVER_VAULT_PASSPHRASE ??
      this.resolveMachineMasterKey();
  ...
  private resolveMachineMasterKey(): string {
    ...
    const machineSeed = `${os.hostname()}:${userInfo.username}:${userInfo.homedir}:tool-evolver-vault-seed`;
