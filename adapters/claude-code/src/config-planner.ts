...
/**
 * Standard server definition for Tool Evolver Gateway in Claude Code.
 */
...
/**
 * Generates planned JSON configuration content adding or updating the tool-evolver MCP server.
 */
export function generatePlannedClaudeConfig(
  currentContent: string | null,
  gatewayUrl: string,
  serverKey = "tool-evolver",
): string {
  ...
  // Configure tool-evolver gateway MCP endpoint
  const serverConfig: ClaudeMcpServerConfig = {
    type: "sse",
    url: gatewayUrl,
  };

  doc.mcpServers[serverKey] = serverConfig;
...
/**
 * Plans an atomic mutation to register the Tool Evolver Gateway in Claude Code's MCP config.
 */
export async function planClaudeMcpConfig(
  ...
  return planConfigMutation({
    ...
    currentContent,
    plannedContent,
    description: "Register Tool Evolver Gateway in Claude Code MCP settings",
  });
}
/**
...
export async function verifyClaudeMcpConfig(
  ...
  try {
    ...
    const entry = doc.mcpServers["tool-evolver"] as ClaudeMcpServerConfig | undefined;
