...
const MANAGED_BLOCK_START = "<!-- tool-evolver:catalog:start -->";
const MANAGED_BLOCK_END = "<!-- tool-evolver:catalog:end -->";
...
export function buildOmpCatalogInstructionsBlock(options: {
  ...
  const serverName = options.serverName ?? "tool-evolver-gateway";
  const lines: string[] = [
    MANAGED_BLOCK_START,
    "",
...
export interface SyncOmpCatalogInstructionsOptions {
  /** Base URL of the tool-evolver cloud backend, e.g. http://127.0.0.1:8080 */
  cloudUrl: string;
  workspaceId: string;
  accountId?: string;
...
/**
 * Upserts the tool-evolver managed block in OMP's append-system-prompt file
...
export async function applyOmpCatalogInstructions(
  ...
  if (hasBlock) {
    ...
    next = existing && existing.trim().length > 0 ? existing.trimEnd() + "\n\n" + block + "\n" : block + "\n";
