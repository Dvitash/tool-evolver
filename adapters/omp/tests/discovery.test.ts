...
describe("OMP Discovery, Installation Probing & Breadcrumbs", () => {
  it("resolves OMP home directory accurately with overrides and defaults", () => {
    ...
    const fromToolEvolverEnv = resolveOmpHome({
      env: { TOOL_EVOLVER_OMP_HOME: "/te/omp" } as unknown as NodeJS.ProcessEnv,
    });
    expect(fromToolEvolverEnv).toBe(path.resolve("/te/omp"));
