...
describe("renderOmpInvocationSnippet", () => {
  it("renders xd:// invocation and docs paths with underscored server name", () => {
    const snippet = renderOmpInvocationSnippet("git_operation_helper", "tool-evolver-gateway");
    expect(snippet).toContain("xd://mcp__tool_evolver_gateway_git_operation_helper");
...
describe("buildOmpCatalogInstructionsBlock", () => {
  it("wraps markdown in managed markers and appends per-tool invocation snippets", () => {
    ...
    expect(block).toContain("<!-- tool-evolver:catalog:start -->");
    expect(block).toContain("<!-- tool-evolver:catalog:end -->");
    expect(block).toContain("## Evolved Tools");
    expect(block).toContain("#### Invocation: `git_operation_helper`");
    expect(block).toContain("xd://mcp__tool_evolver_gateway_git_operation_helper");
  });
});
...
describe("applyOmpCatalogInstructions", () => {
  it("creates the append-system file with the managed block", async () => {
    ...
    try {
      ...
      expect(content).toContain("<!-- tool-evolver:catalog:start -->");
      expect(content).toContain("xd://mcp__tool_evolver_gateway_alpha_tool");
    } finally {
  ...
  it("removes the managed block when markdown is empty", async () => {
    ...
    try {
      ...
      const content = await fsp.readFile(target, "utf8");
      expect(content).not.toContain("tool-evolver:catalog");
    } finally {
      await fsp.rm(tmpDir, { recursive: true, force: true });
    }
  });
...
describe("syncOmpCatalogInstructions", () => {
  it("fetches markdown from the cloud route and writes invocation snippets", async () => {
    ...
    try {
      ...
      const content = await fsp.readFile(target, "utf8");
      expect(content).toContain("xd://mcp__tool_evolver_gateway_git_operation_helper");
    } finally {
      await fsp.rm(tmpDir, { recursive: true, force: true });
    }
  });
