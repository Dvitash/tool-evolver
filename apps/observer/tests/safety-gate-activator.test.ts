import { SAFETY_GATE_ERROR_CODES, UNSAFE_DEV_OVERRIDE_ENV_VAR } from "@tool-evolver/contracts";
...
describe("Observer Safety Gate Activator & Worker Environment Sanitization", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.TOOL_EVOLVER_UNSAFE_DEV_MODE;
  });
  ...
  describe("WorkerSupervisor buildSanitizedEnv", () => {
    it("strictly strips TOOL_EVOLVER_UNSAFE_* from inherited process environment", () => {
      process.env[UNSAFE_DEV_OVERRIDE_ENV_VAR] = "true";
      process.env.TOOL_EVOLVER_UNSAFE_DEBUG = "1";
      ...
      expect(env.TOOL_EVOLVER_UNSAFE_DEBUG).toBeUndefined();
    });

    it("strictly strips TOOL_EVOLVER_UNSAFE_* passed in customEnv", () => {
      const customEnv = {
        NORMAL_PARAM: "valid",
        [UNSAFE_DEV_OVERRIDE_ENV_VAR]: "1",
        TOOL_EVOLVER_UNSAFE_INJECT: "malicious",
      };
      ...
      expect(env[UNSAFE_DEV_OVERRIDE_ENV_VAR]).toBeUndefined();
      expect(env.TOOL_EVOLVER_UNSAFE_INJECT).toBeUndefined();
    });
  });
      ...
      const closedGate = {
        canExecuteTool: () => ({
          ...
          refusal: {
            ...
            remediation:
              "Run 'tool-evolver doctor --repair' to generate a valid safety attestation.",
