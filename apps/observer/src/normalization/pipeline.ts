...
import type { LocalDatabaseConnection, SessionRepository, SyncRepository } from "@tool-evolver/db";
...
export class NormalizationPipeline {
  ...
  async processIntermediateEvent(
    ...
    // 6. Record in deduplicator
    this.deduplicator.recordEvent(validEvent, dedupOutcome.contentHash);

    // 7. Update session causal tracking state
    sessionMap.set(causalSequence, validEvent.eventId);
    this.sessionState.set(sessionId, {
      lastSequence: Math.max(state.lastSequence, causalSequence),
    });

    // 8. Persist to SQLite (if repository or db connection available)
