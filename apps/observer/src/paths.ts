...
export function resolvePaths(options: PathResolutionOptions = {}): DaemonPaths {
  ...
  if (explicitHome) {
    ...
    baseConfigDir = path.join(userHome, "Library", "Application Support", "tool-evolver");
    baseDataDir = path.join(userHome, "Library", "Application Support", "tool-evolver");
    ...
    baseHomeDir = path.join(userHome, ".tool-evolver");
    baseConfigDir = path.join(appData, "tool-evolver");
    baseDataDir = path.join(localAppData, "tool-evolver");
    baseStateDir = path.join(localAppData, "tool-evolver", "state");
    baseLogDir = path.join(localAppData, "tool-evolver", "logs");
  ...
  let socketPath = options.socketPath ?? env.TOOL_EVOLVER_SOCKET_PATH ?? defaultSocketPath;
  ...
  const lockFilePath = path.resolve(
    options.lockFilePath ?? env.TOOL_EVOLVER_LOCK_FILE ?? path.join(stateDir, "daemon.lock"),
  );
  const pidFilePath = path.resolve(
    options.pidFilePath ?? env.TOOL_EVOLVER_PID_FILE ?? path.join(stateDir, "daemon.pid"),
  );
  const tokenFilePath = path.resolve(
    options.tokenFilePath ?? env.TOOL_EVOLVER_TOKEN_FILE ?? path.join(stateDir, "auth.token"),
  );
  const configFile = path.resolve(
    options.configFile ?? env.TOOL_EVOLVER_CONFIG_FILE ?? path.join(configDir, "config.json"),
  );
