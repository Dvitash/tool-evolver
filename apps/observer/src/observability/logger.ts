...
const COMMON_SECRET_PATTERNS: Array<{ regex: RegExp; replacement: string }> = [
  ...
  {
    regex: /Bearer\s+[A-Za-z0-9_\-\.]{15,}/g,
 ...
 */
export function redactSecrets(data: unknown, currentKey?: string): unknown {
  if (data === null || data === undefined) {
    return data;
  }

  if (typeof data === "string") {
    // If the containing key is already sensitive, redact directly
    ...
    // High entropy token heuristic (> 4.5 entropy and length >= 28)
  ...
  if (Array.isArray(data)) {
    return data.map((item) => redactSecrets(item, currentKey));
  }
  ...
  if (typeof data === "object") {
    ...
    for (const [key, value] of Object.entries(data as Record<string, unknown>)) {
      if (SENSITIVE_KEY_PATTERN.test(key)) {
        ...
        output[key] = redactSecrets(value, key);
      }
    }
    return output;
  }
