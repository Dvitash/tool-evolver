...
describe("logout command", () => {
  ...
  const tokenFilePath = path.join(homeDir, ".tool-evolver", "state", "device-token.json");
...
describe("uninstall command & harness cleanup", () => {
  ...
  it("removes Tool Evolver MCP configuration from all detected harnesses", async () => {
    const initialClaude = JSON.stringify({
      mcpServers: {
        "tool-evolver": { url: "http://localhost:9400" },
...
`;
    ...
    // Verify Claude config has tool-evolver removed but other-tool preserved
    const updatedClaude = JSON.parse((await fsBridge.readFile(claudePath))!);
    expect(updatedClaude.mcpServers["tool-evolver"]).toBeUndefined();
    ...
    expect(updatedCodex).not.toContain("[mcp_servers.tool-evolver]");
    expect(updatedCodex).toContain("[mcp_servers.other]");

    // Verify OMP config has tool-evolver removed
    const updatedOmp = JSON.parse((await fsBridge.readFile(ompPath))!);
    expect(updatedOmp.mcpServers["tool-evolver"]).toBeUndefined();
  });
