...
describe("Cross-Platform Service Lifecycle Suite", () => {
  describe("Service Generators & Syntax Validation", () => {
    it("generates and validates a complete Linux systemd user unit file", () => {
      const unitContent = generateSystemdUnit({
        serviceName: "tool-evolver-test",
        description: "Tool Evolver Unit Test Daemon",
        daemonPath: "/home/user/.tool-evolver/bin/daemon",
        nodePath: "/usr/bin/node",
        homeDir: "/home/user",
        toolEvolverHome: "/home/user/.tool-evolver",
        restartSec: 5,
        enableHardening: true,
        env: {
          TOOL_EVOLVER_ENV: "staging",
        },
      });
      ...
      expect(unitContent).toContain("ExecStart=/usr/bin/node /home/user/.tool-evolver/bin/daemon");
      expect(unitContent).toContain("RestartSec=5");
      expect(unitContent).toContain("NoNewPrivileges=yes");
      expect(unitContent).toContain('Environment="TOOL_EVOLVER_ENV=staging"');
    ...
    it("generates and validates a valid macOS launchd plist file", () => {
      const plistContent = generateLaunchdPlist({
        serviceName: "com.toolevolver.daemon",
        daemonPath: "/Users/test/.tool-evolver/bin/daemon",
        nodePath: "/usr/local/bin/node",
        homeDir: "/Users/test",
        toolEvolverHome: "/Users/test/.tool-evolver",
        logDir: "/Users/test/Library/Logs/tool-evolver",
        env: {
          TOOL_EVOLVER_STAGE: "release",
        },
      });

      expect(plistContent).toContain("<!DOCTYPE plist PUBLIC");
      expect(plistContent).toContain("<key>Label</key>");
      expect(plistContent).toContain("<string>com.toolevolver.daemon</string>");
      ...
      expect(plistContent).toContain("/Users/test/Library/Logs/tool-evolver/daemon.log");
      expect(plistContent).toContain("<key>TOOL_EVOLVER_STAGE</key>");
    ...
    it("generates and validates a WSL supervisor fallback runner script", () => {
      const scriptContent = generateWslFallbackScript({
        daemonPath: "/home/wsluser/.tool-evolver/bin/daemon",
        nodePath: "/usr/bin/node",
        homeDir: "/home/wsluser",
        toolEvolverHome: "/home/wsluser/.tool-evolver",
        env: {
  ...
  describe("Linux systemd Service Manager Lifecycle", () => {
    it("performs full install, enable, start, status, stop, and uninstall cycle", async () => {
      ...
      const manager = new SystemdUserServiceManager({
        ...
        toolEvolverHome: "/home/testuser/.tool-evolver",
        fsBridge,
        runner,
      });
  ...
  describe("macOS launchd Service Manager Lifecycle", () => {
    it("performs full launchd plist creation, load, status, unload, and cleanup", async () => {
      const fsBridge = createMockFsBridge();
      const runner = createMockRunner({
        "launchctl list com.tool-evolver.daemon": () => ({
      ...
      const manager = new LaunchdUserServiceManager({
        ...
        toolEvolverHome: "/Users/testuser/.tool-evolver",
        fsBridge,
        runner,
      });
