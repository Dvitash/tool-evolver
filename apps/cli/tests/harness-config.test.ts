...
describe("HarnessConfigOrchestrator", () => {
  it("configures Claude Code, Codex CLI, and OMP in a clean environment", async () => {
    ...
    expect(claudeJson.mcpServers["tool-evolver"].url).toBe("http://127.0.0.1:9400/mcp/sse");
    ...
    const ompContent = await bridge.readFile(`${home}/.omp/config.json`);
    expect(ompContent).not.toBeNull();
    const ompJson = JSON.parse(ompContent ?? "{}");
    expect(ompJson.mcpServers["tool-evolver-gateway"].url).toBe("http://127.0.0.1:9400/mcp/sse");
  });
