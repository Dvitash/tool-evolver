...
/**
 * Standard paths for Tool Evolver runtime, state, logs, sockets, and configuration.
 */
 ...
 * - macOS: ~/Library/Application Support/ToolEvolver, ~/Library/Caches/tool-evolver, ~/Library/Logs/tool-evolver
 * - Linux: ~/.local/share/tool-evolver, ~/.config/tool-evolver, ~/.local/state/tool-evolver
...
export function resolvePlatformPaths(options: PlatformPathOptions = {}): PlatformPaths {
  ...
  const explicitHome = options.toolEvolverHome ?? env.TOOL_EVOLVER_HOME;
  ...
  if (explicitHome) {
    ...
    baseHomeDir = path.join(userHome, ".tool-evolver");
    // Standard macOS Library locations (supporting ToolEvolver and tool-evolver)
    baseConfigDir = path.join(userHome, "Library", "Application Support", "ToolEvolver");
    baseDataDir = path.join(userHome, "Library", "Application Support", "ToolEvolver");
    baseStateDir = path.join(userHome, "Library", "Caches", "tool-evolver");
    baseLogDir = path.join(userHome, "Library", "Logs", "tool-evolver");
    defaultSocketPath = path.join(baseStateDir, "daemon.sock");
    ...
    baseHomeDir = path.join(userHome, ".tool-evolver");
    baseConfigDir = path.join(appData, "ToolEvolver");
    baseDataDir = path.join(localAppData, "ToolEvolver");
    baseStateDir = path.join(localAppData, "ToolEvolver", "state");
    baseLogDir = path.join(localAppData, "ToolEvolver", "logs");
    defaultSocketPath = "\\\\.\\pipe\\tool-evolver-daemon";
  } else {
    ...
    baseConfigDir = xdgConfig
      ? path.join(xdgConfig, "tool-evolver")
      : path.join(userHome, ".config", "tool-evolver");
    baseDataDir = xdgData
      ? path.join(xdgData, "tool-evolver")
      : path.join(userHome, ".local", "share", "tool-evolver");
    baseStateDir = xdgState
      ? path.join(xdgState, "tool-evolver")
      : path.join(userHome, ".local", "state", "tool-evolver");
    baseLogDir = path.join(baseStateDir, "logs");

    if (xdgRuntime) {
      defaultSocketPath = path.join(xdgRuntime, "tool-evolver.sock");
    } else {
      defaultSocketPath = path.join(baseStateDir, "daemon.sock");
    }
  }
  ...
  const configDir = path.resolve(options.configDir ?? env.TOOL_EVOLVER_CONFIG_DIR ?? baseConfigDir);
  const dataDir = path.resolve(options.dataDir ?? env.TOOL_EVOLVER_DATA_DIR ?? baseDataDir);
  const stateDir = path.resolve(options.stateDir ?? env.TOOL_EVOLVER_STATE_DIR ?? baseStateDir);
  const logDir = path.resolve(options.logDir ?? env.TOOL_EVOLVER_LOG_DIR ?? baseLogDir);
  const socketPath = path.resolve(
    options.socketPath ?? env.TOOL_EVOLVER_SOCKET_PATH ?? defaultSocketPath,
  );
  ...
  if (platform.isWsl) {
    ...
    wslHostConfig = {
      windowsAppDataDir: `${mntRoot}/c/Users/${winUser}/AppData/Roaming/ToolEvolver`,
      windowsLocalAppDataDir: `${mntRoot}/c/Users/${winUser}/AppData/Local/ToolEvolver`,
