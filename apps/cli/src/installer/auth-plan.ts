...
/**
 * Privacy and observability configuration for Tool Evolver installation.
 */
...
/**
 * Generates the default capability envelope for Tool Evolver runtime operations.
 */
...
export function formatAuthPlanForDisplay(plan: AuthorizationPlan): string {
  ...
  lines.push("================================================================================");
  lines.push("                    TOOL EVOLVER AUTHORIZATION PLAN                             ");
  lines.push("================================================================================");
  ...
  lines.push(
    `   - Telemetry:              ${plan.privacy.telemetryEnabled ? "ENABLED" : "DISABLED (Default opt-out)"}`,
  );
  lines.push("");
...
export async function validateAuthorization(
  ...
  // If loaded from explicit capabilities file or explicit autoApprove flag in non-interactive mode
  if (options.capabilitiesFile) {
    plan.granted = true;
  ...
  // Interactive prompt
  if (options.promptFn) {
    const approved = await options.promptFn(
      "Do you authorize Tool Evolver with the capability envelope and privacy plan displayed above? (y/N): ",
    );
    if (!approved) {
  ...
  // Default if interactive without custom prompt function
  plan.granted = true;
  plan.grantedAt = new Date().toISOString();
  plan.grantedBy = "interactive_user";
  return plan;
