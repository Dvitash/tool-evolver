...
async function main() {
  ...
  console.log(`[Migrate] Running migration command '${command}' against ${config.database.database}...`);

  try {
    if (command === "up") {
      const result = await runMigrations(pool);
      console.log(`[Migrate] Successfully applied ${result.appliedCount} migration(s). Current version: ${result.currentVersion}`);
    } else if (command === "down") {
      const targetVersion = process.argv[3] ? Number(process.argv[3]) : 0;
      const result = await rollbackMigration(pool, { targetVersion });
      console.log(`[Migrate] Successfully rolled back ${result.rolledBackCount} migration(s). Current version: ${result.currentVersion}`);
    } else if (command === "status") {
      const statuses = await getMigrationStatus(pool);
      console.log("[Migrate] Migration status:");
      for (const s of statuses) {
        console.log(`  - Version ${s.version}: ${s.name} [${s.applied ? "APPLIED" : "PENDING"}] (applied at: ${s.appliedAt ?? "N/A"})`);
      }
    } else {
