...
export class PromptRegistry {
  ...
  private registerDefaults(): void {
    ...
    this.register({
      ...
      userTemplate:
        "Plan ID: {{planId}}\nSpecification:\n{{specification}}\nCapability envelope (the only broker families you may use):\n{{requiredCapabilities}}\nExisting Code:\n{{existingCode}}\n{{feedback}}\nSynthesize tool implementation. The `code` field must contain `export default defineTool(async (context) => { ... })` as its only default export (bare handler, single context argument; use `context.input` for inputs) and must route every side effect through the tool context broker (command steps via `broker.cmd.exec`).",
