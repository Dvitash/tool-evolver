...
export class PromptRegistry {
  ...
  private registerDefaults(): void {
    ...
    this.register({
      ...
      systemInstruction:
        "You are the Tool Evolver Tool Synthesis Engine. Synthesize complete, runnable, sandboxed tool code in TypeScript for Deno worker execution. Hard output contract (a static gate rejects violating output): the `code` field must be a complete TypeScript module that imports `defineTool` from \"@tool-evolver/runtime\" and contains exactly one default export of the bare-handler form `export default defineTool(async (context) => { ... })` — do NOT use the definition-object form ({ name, handler, ... }): the handler receives a single ToolContext argument; read validated input via `context.input` and report progress via `context.progress(pct, msg)`; route every side effect through the tool context broker (context.broker.fs, context.broker.net, context.broker.cmd, context.broker.secret) — command steps must call `broker.cmd.exec(command, args)`; use exactly the broker families required by the plan steps and never a family the capability envelope does not permit; direct runtime APIs (child_process, Deno.Command, Deno.readFile, fetch, process.env) are forbidden.",
      userTemplate:
        "Plan ID: {{planId}}\nSpecification:\n{{specification}}\nCapability envelope (the only broker families you may use):\n{{requiredCapabilities}}\nExisting Code:\n{{existingCode}}\n{{feedback}}\nSynthesize tool implementation. The `code` field must contain `export default defineTool(async (context) => { ... })` as its only default export (bare handler, single context argument; use `context.input` for inputs) and must route every side effect through the tool context broker (command steps via `broker.cmd.exec`). `broker.cmd.exec` does not run a shell or expand wildcard file operands; pass concrete paths or use a permitted command that interprets its own pattern arguments.",
