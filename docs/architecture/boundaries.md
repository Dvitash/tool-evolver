...
This document specifies the trust boundaries, process isolation model, inter-process communication (IPC) mechanisms, and network security boundaries across the Tool Evolver V1 architecture.

## 1. Trust Boundaries

Tool Evolver defines five distinct, non-overlapping trust zones:
...
|         v                                                     |
|  +----------------------------------------------------------+ |
|  | PID 10240: Tool Evolver Daemon (Node.js LTS)             | |
...
| :--- | :--- | :--- | :--- | :--- |
| **Harness <-> Gateway** | `stdio` (pipes) or Unix Domain Socket (`~/.tool-evolver/gateway.sock`) | Model Context Protocol (MCP / JSON-RPC 2.0) | Bidirectional | Socket permissions (`0600`), message size validation |
