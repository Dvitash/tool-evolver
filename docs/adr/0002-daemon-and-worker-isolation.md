...
The Tool Evolver local architecture must support two distinct operational requirements:
...
2. **Isolated Tool Worker Sandboxes (Execution Plane)**:
   ...
   - The Deno runtime binary is version-pinned and managed by the Tool Evolver toolchain, guaranteeing identical behavior across platforms.
...
- Unit tests in `@tool-evolver/runtime` verify that worker crashes (`process.exit(1)`, syntax errors, infinite loops) are caught and reported as tool execution errors without affecting the gateway daemon.
