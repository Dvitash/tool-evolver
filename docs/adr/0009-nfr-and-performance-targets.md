...
We decide on the following explicit numeric targets for the Tool Evolver V1 system:
...
- Minimal resource consumption ensures Tool Evolver runs seamlessly on developer laptops.
...
- The Vitest performance test suite executes micro-benchmarks against `@tool-evolver/gateway` and `@tool-evolver/runtime` to enforce these thresholds.
