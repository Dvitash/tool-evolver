...
Tool Evolver provides first-class, verified adapters for three AI harnesses:
...
All harness interactions are mediated through dedicated adapter packages (`@tool-evolver/adapter-claude-code`, `@tool-evolver/adapter-codex`, `@tool-evolver/adapter-omp`), which implement the common `@tool-evolver/harness-contracts` interface.

### 2. Supported Operating System Platforms

Tool Evolver V1 officially supports:
...
|                              v                                |
|  +----------------------------------------------------------+ |
|  | @tool-evolver/harness-contracts (Standard Normalized API)| |
|  +---------------------------+------------------------------+ |
|                              |                                |
|                              v                                |
|  +----------------------------------------------------------+ |
|  | @tool-evolver/gateway (Local MCP Gateway Engine)         | |
|  +----------------------------------------------------------+ |
+---------------------------------------------------------------+
```
...
### Negative / Trade-offs
- Windows users who do not use WSL2 cannot run Tool Evolver natively in V1.

### Mitigations
- Clear documentation and CLI error messages detect native Windows execution and guide users to launch within WSL2.
- The CLI provides an automatic configuration command (`tool-evolver setup <harness>`) that auto-detects installed harnesses and writes the appropriate MCP configuration snippet.

## Compliance and Verification

- CI pipeline runs automated test suites across Ubuntu Linux (`x86_64`), macOS (`arm64`), and Ubuntu on WSL2.
- Monorepo package boundary rules ensure that harness-specific adapters remain decoupled and only interact via `@tool-evolver/harness-contracts`.
