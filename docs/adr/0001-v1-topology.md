...
### Negative / Trade-offs
- Requires a persistent local background daemon process (`tool-evolver` daemon).
...
- Provide a CLI fallback mode (`tool-evolver gateway --standalone`) for ad-hoc debugging.

## Compliance and Verification

- The boundary checker (`scripts/check-boundaries.mjs`) ensures harness adapters only depend on `@tool-evolver/protocol` and `@tool-evolver/contracts`.
