...
export function generateMarkdownDocs(
  ...
  const title = options.title || "Tool Evolver Contract Specifications";
  lines.push(`# ${title}\n`);
...
export function generateFullContractCatalogDoc(): string {
  const allSchemas: Record<string, z.ZodTypeAny> = {
    ...
    CandidateTriggerReason: CandidateTriggerReasonSchema,
  ...
  return generateMarkdownDocs(allSchemas, {
    title: "Tool Evolver Core Schema & Contract Catalog",
    description:
