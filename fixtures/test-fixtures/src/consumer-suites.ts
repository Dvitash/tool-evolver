...
export interface StorageAdapter {
  ...
  getEvolutionCandidate(candidateId: string): Promise<EvolutionCandidate | null>;
...
export function defineStorageRoundTripSuite(options: StorageSuiteOptions) {
  return async (ctx: TestContext = defaultAssert) => {
    ...
    try {
      ...
      // 2. Candidate Round Trip
      await adapter.saveEvolutionCandidate(validEvolutionCandidate);
      ...
      // 4. Deployment Record Round Trip
...
export interface RegistryAdapter {
  ...
  submitCandidate(candidate: EvolutionCandidate): Promise<void>;
  updateCandidateState(candidateId: string, newState: EvolutionCandidate["state"]): Promise<void>;
  getCandidate(candidateId: string): Promise<EvolutionCandidate | null>;
...
export function defineRegistrySuite(options: RegistrySuiteOptions) {
  return async (ctx: TestContext = defaultAssert) => {
    ...
    try {
      ...
      await registry.submitCandidate(validEvolutionCandidate);
      let cand = await registry.getCandidate(validEvolutionCandidate.id);
      ctx.assert(cand !== null, "Candidate registered");
      ctx.assertEqual(cand?.state, "detected", "Initial state is detected");

      await registry.updateCandidateState(validEvolutionCandidate.id, "synthesizing");
      cand = await registry.getCandidate(validEvolutionCandidate.id);
      ctx.assertEqual(cand?.state, "synthesizing", "State updated to synthesizing");

      await registry.updateCandidateState(validEvolutionCandidate.id, "synthesized");
      cand = await registry.getCandidate(validEvolutionCandidate.id);
      ctx.assertEqual(cand?.state, "synthesized", "State updated to synthesized");
